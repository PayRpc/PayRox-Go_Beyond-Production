  ...
  it("validates deployedBytecode length", () => {
    ...
    (function walk(dir: string) {
      ...
      for (const e of fs.readdirSync(dir)) {
        ...
        if (fs.statSync(_p).isDirectory()) walk(_p);
        else if (_p.endsWith(".json")) files.push(_p);
      }
    })(artifactsRoot);

    for (const f of files) {
      const _art = JSON.parse(fs.readFileSync(f, "utf8"));
      const code: string = _art.deployedBytecode || "0x";
      const _bytes = code === "0x" ? 0 : (code.length - 2) / 2;
      expect(_bytes).to.be.lte(
        _limit,
        `${path.basename(f)} exceeds EIP-170 (${_bytes}B)`,
      );
    }
  });
});
