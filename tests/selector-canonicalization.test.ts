...
describe('Selector Canonicalization Tests', function() {
    it('should canonicalize function selectors correctly', function() {
        ...
        const _hash = ethers.keccak256(ethers.toUtf8Bytes(signature));
        const _selector = hash.slice(0, 10); // First 4 bytes (8 hex chars + 0x)
    ...
    it('should handle selector collisions detection', function() {
        ...
        const _selector1 = ethers.keccak256(ethers.toUtf8Bytes(sig1)).slice(0, 10);
        const _selector2 = ethers.keccak256(ethers.toUtf8Bytes(sig2)).slice(0, 10);
