...
class DiamondDeployer {
  ...
  private readonly deployedFacets = new Map<string, Contract>()
  ...
  private loadManifest (): void {
    ...
    this.manifest = JSON.parse(
      fs.readFileSync(this.config.manifestPath, 'utf-8')
    )
  }
  ...
  private async deployFacets (): Promise<void> {
    ...
    for (const [facetName, facetData] of Object.entries(this.manifest.facets)) {
      ...
      console.log(
        `    ✅ ${facetName} deployed to: ${await facet.getAddress()}`
      )
      ...
      if (sizeBytes > 24576) {
        throw new Error(
          `❌ ${facetName} exceeds EIP-170 limit: ${sizeBytes} > 24576 bytes`
        )
      }
    ...
    for (const [facetName, facet] of this.deployedFacets.entries()) {
      ...
      for (const selector of selectors) {
        if (loupeSelectors.includes(selector)) {
          throw new Error(
            `❌ Facet ${facetName} implements forbidden loupe function: ${selector}`
          )
        }
      }
    }
        ...
        const leaf = hre.ethers.keccak256(
          hre.ethers.AbiCoder.defaultAbiCoder().encode(
            ...
            [
              selector,
              facetData.address,
              hre.ethers.keccak256(hre.ethers.toUtf8Bytes(facetName))
            ]
          )
        )
        leaves.push(leaf)
  ...
  private getFunctionSelectors (contract: Contract): string[] {
    ...
    for (const func of Object.values(
      (contract.interface as any).functions || {}
    )) {
  ...
  await hre.ethers.getSigners()
    ...
    try {
      ...
      console.log(
        `  ✅ Diamond deployed at: ${await (diamond as any).getAddress()}`
      )
    } catch (error) {
...
export { DiamondDeployer, type DeploymentConfig }
