        ...
        if (sub.type === 'StateVariableDeclaration') {
          for (const v of sub.variables) {
            ...
            try {
              ...
              console.warn(`Failed to extract type for ${v.name}: ${err.message}`);
...
function collectStateVarsLinearized(ast, targetContract) {
  ...
  for (const n of linear) {
    ...
    for (const v of entry.vars) {
      ...
      seen.add(v.name);
      acc.push({ name: v.name, type: v.type, from: n, dropped: false, declRange: v.declRange });
    }
  }
  ...
  return { contractName: root, vars: acc, baseOrder: linear.filter((x) => x !== root), varDecls };
}
...
function makeInitializerStub(constructors, contractName, libName) {
  ...
  const constructorBody =
    constructors[0].node && constructors[0].node.body
...
function extractConstructorBody(constructorNode) {
  ...
  if (!constructorNode.body || !constructorNode.body.statements) {
