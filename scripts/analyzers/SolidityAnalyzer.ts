...
export class SolidityAnalyzer {
  ...
  private estimateFunctionSize (functionNode: any, sourceCode: string): number {
    ...
    const endIdx =
      typeof location.end === 'number' ? location.end : sourceCode.length
   ...
   */
  generateManifestEntries (contract: ParsedContract): Array<Record<string, unknown>> {
  ...
  generateFacetAnalysisReport (contract: ParsedContract): {
    ...
    const facetEntries: Array<[string, any[]]> = contract.facetCandidates
      ? (Array.from((contract.facetCandidates as any).entries()))
      : []
  ...
  initializeCLI (): Command {
        ...
        async (
          ...
          try {
            ...
            if (!fs.existsSync(contractPath)) {
              console.error(
                `Error: Contract file not found at ${contractPath}`
              )
              process.exit(1)
            }
            ...
            // Analyze contract
            const analysis = await this.parseContract(
              sourceCode,
            ...
            if (options.verbose) {
              ...
              console.log(
                `- Deployment strategy: ${analysis.deploymentStrategy}`
              )
    ...
    program
      .command('refactor')
      .description(
        'Plan a non-destructive refactor for a solidity file (dry-run)'
      )
    ...
    program
      ...
      .option(
        '-g, --gas-limit <gas>',
        'Maximum gas limit for gas-based chunking'
      )
      .option('--contract-name <name>', 'Specific contract name to analyze')
      .action(
        async (
          ...
          try {
            ...
            console.log(
              `Total estimated size: ${chunkPlan.totalEstimatedSize} bytes`
            )

            if (chunkPlan.optimization) {
              console.log(
                `Optimization efficiency: ${chunkPlan.optimization.efficiency}`
              )
              console.log(
                `Gas optimized: ${chunkPlan.optimization.gasOptimized}`
              )
              console.log(
                `Dependency score: ${chunkPlan.optimization.dependencyScore}`
              )

              if (chunkPlan.optimization.recommendations.length > 0) {
                console.log('Recommendations:')
                chunkPlan.optimization.recommendations.forEach(
                  (rec: string) => {
                    console.log(`  - ${rec}`)
                  }
                )
  ...
  private generateChunkPlan (
    ...
    switch (options.strategy) {
      ...
      case 'gas':
        chunks = this.planChunksByGasUsage(
          chunkFunctions,
      ...
      case 'function':
      default:
        chunks = this.planChunksByFunction(
          chunkFunctions,
          options.maxChunkSize
        )
        break
  ...
  private generateManifest (
    ...
    const verification = this.calculateManifestVerification(
      chunkPlan.chunks,
      routes
    )
  ...
  private generateMarkdownReport (analysis: ParsedContract): string {
    ...
    const facetEntriesReport: Array<[string, any[]]> = analysis.facetCandidates
      ? (Array.from((analysis.facetCandidates as any).entries()))
      : []
  ...
  async parseContractUltraLightweight (
    ...
    try {
      ...
      const mkFacet = (name: string, list: string[]): any => {
        ...
        const estimatedSize = list.reduce(
          (sum, n) => sum + (fnSizes[n] || 0),
          0
        )
