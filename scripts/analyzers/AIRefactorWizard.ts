...
import { type FunctionInfo, type ParsedContract } from '../types/index'
...
const selectorOf = (sig: string) =>
  '0x' + keccak256(toUtf8Bytes(sig)).slice(2, 10)
...
function splitByMax (
  name: string,
  fns: FunctionInfo[]
): Array<{ name: string, fns: FunctionInfo[] }> {
...
function bucket (_c: ParsedContract, f: FunctionInfo) {
  ...
  if (
    n.includes('router') ||
    n.includes('dispatcher') ||
    n.includes('extcodehash')
  ) { return 'CoreFacet' }
  if (
    n.includes('twap') ||
    n.includes('price') ||
