...
async function main () {
  ...
  const maxChunkSize =
    maxChunkSizeFlag > -1 ? Number(args[maxChunkSizeFlag + 1]) : 24576
  const strategy =
    strategyFlag > -1
  ...
  const result = await analyzer.refactorContract(src, {
    maxChunkSize,
    strategy
  })
  ...
  console.log(
    `Generated ${result.patches.length} facet stub(s). Summary: ${result.summary}`
  )
}

main().catch((err) => {
  console.error(
    'Error generating facets:',
