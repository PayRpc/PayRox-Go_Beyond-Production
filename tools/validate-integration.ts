...
function sigFromABI(fn: ABIFragment): string {
  const _inputs = (fn.inputs || []).map(i => i.type).join(",");
  return `${fn.name}(${inputs})`;
}

function sizeFromArtifact(a: Artifact): number {
  const _rt = a.deployedBytecode || "0x";
...
function normalizeManifest(m: Manifest): Record<string, string[]> {
  ...
  for (const [k, v] of Object.entries(m.facets || {})) {
    const _sels = Array.isArray(v) ? v : (v as any).selectors;
...
async function main() {
  ...
  const _manifestPath = path.resolve(root, "payrox-manifest.json");
  if (!fs.existsSync(manifestPath)) {
    fail("payrox-manifest.json not found; generate it first (tools/generate-manifest.js).");
  }
  const _manifest = loadJSON<Manifest>(manifestPath);
  const _mf = normalizeManifest(manifest);

  // 1) Load all facet artifacts
  const _facetArtifacts = await glob("artifacts/contracts/facets/**/*.json", { cwd: root, nodir: true });
  ...
  for (const rel of facetArtifacts) {
    const _p = path.resolve(root, rel);
    const _art = loadJSON<Artifact>(p);

    const _name = art.contractName || path.basename(p, ".json");
    const _isFacet = (art.sourceName || "").includes("/facets/");
    const _isInterface = name.startsWith("I") && name !== "IAntiBotFacet"; // Skip interfaces except actual facets like IAntiBotFacet
    
    if (!isFacet || isInterface) continue; // ignore libs/interfaces caught by glob

    // Build selector set from ABI
    const _fnFrags = (art.abi || []).filter(f => f.type === "function") as ABIFragment[];
    const _sigs = fnFrags.map(sigFromABI);
    const _sels = sigs.map(selectorFromSig);
    ...
    for (const f of fnFrags) {
      ...
      const _sel = selectorFromSig(sigFromABI(f));
    ...
    const _size = sizeFromArtifact(art);
    if (size > EIP170) {
  ...
  for (const [facet, mSelectors] of Object.entries(mf)) {
    ...
    const _cset = new Set(compiledSelectors[facet].map(s => s.toLowerCase()));
    const _mset = new Set(mSelectors.map(s => s.toLowerCase()));
  ...
  const _wizardPath = path.resolve(root, "tools/AIInterfaceWizard.ts");
  if (fs.existsSync(wizardPath)) {
    const _text = fs.readFileSync(wizardPath, "utf8");
  ...
  const _tsFiles = await glob("scripts/**/*.ts", { cwd: root });
  for (const tsFile of tsFiles) {
    const _tsPath = path.resolve(root, tsFile);
    const _content = fs.readFileSync(tsPath, "utf8");
  ...
  const _saltTS = path.resolve(root, "scripts/cross-network-registry.ts");
  if (fs.existsSync(saltTS)) {
    const _text = fs.readFileSync(saltTS, "utf8");
    const _hasPacked = /ethers\.solidityPacked\s*\(/.test(text);
    const _hasKeccak = /ethers\.keccak256\s*\(/.test(text);
    if (!(hasPacked && hasKeccak)) {
      warnings.push("cross-network-registry.ts: expected solidityPacked + keccak256 for universalSalt.");
    }
  }
  const _saltSol = await glob("contracts/**/SaltPolicyLib.sol", { cwd: root });
  if (saltSol.length === 0) {
  ...
  const _routerPath = path.resolve(root, "contracts/Proxy/PayRoxProxyRouter.sol");
  if (fs.existsSync(routerPath)) {
    const _txt = fs.readFileSync(routerPath, "utf8");
