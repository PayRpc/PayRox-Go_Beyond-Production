...
import { ICustomerFacet } from "../interfaces/ICustomerFacet.sol";
import { CustomerStorage as S } from "../libraries/CustomerStorage.sol";

contract CustomerFacet is ICustomerFacet {
    /* Errors */
    error Paused();
    error NotAdmin();
    error NotOperator();
    ...
    bytes32 public constant OPERATOR_ROLE = keccak256("Customer_OPERATOR");
    ...
    function initializeCustomer(address operator) external onlyAdmin {
        if (operator == address(0)) revert ZeroAddress();
        S.Layout storage l = S.layout();
        if (l.initialized) revert AlreadyInitialized();
        l.initialized = true;
        l.operator = operator;
        emit CustomerInitialized(operator);
    }
    ...
    function setConfig(uint256 newValue) external whenNotPaused onlyOperator {
        ...
        emit CustomerConfigSet(newValue, msg.sender);
    }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == 0x01ffc9a7 /* ERC165 */ ||
               interfaceId == type(ICustomerFacet).interfaceId;
    }
        ...
        returns (string memory name, string memory version, bytes4[] memory selectors)
    {
        ...
        selectors[0] = this.initializeCustomer.selector;
